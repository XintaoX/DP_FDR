...
one_iteration_example1<-function(location = 1,
  ...
  for (i in 1:peelm) {
    ...
    newqp[i] <- qp[n_index[i]] + rlaplace(n = 1, location = 0, scale = lambda)
    ind[n_index[i]] <- FALSE
  }
...
to.csv <- function(results){
  tocsv <- NULL
  for (i in 1:ncol(results)) {
    tocsv <- cbind(tocsv,unlist(results[,i]))
  }
  return(tocsv)
}
...
set.seed(951004)
l1m3.5result <-  foreach(mc = 1:100, .combine = cbind,.packages = c("VGAM")) %dopar% one_iteration_example1(mu00 = 2.5)
...
set.seed(951004)
l1m4.5result <-  foreach(mc = 1:100, .combine = cbind,.packages = c("VGAM")) %dopar% one_iteration_example1(mu00 = 3.5)
...
set.seed(951004)
l1m5.5result <-  foreach(mc = 1:100, .combine = cbind,.packages = c("VGAM")) %dopar% tryCatch(one_iteration_example1(mu00 = 4.5),error = function(e) rep(NA_real_, p))
...
write.csv(to.csv(l1m3.5result),"./result/l1m3.5result1.csv")
write.csv(to.csv(l1m4.5result),"./result/l1m4.5result1.csv")
write.csv(to.csv(l1m5.5result),"./result/l1m5.5result1.csv")
write.csv(result1,"./result/let_seting_example1_fdr.csv")
write.csv(result2,"./result/let_seting_example1_power.csv")

result1<-NULL
result2<-NULL
###alpha case mu00=2.5,location=2
set.seed(951004)
l1m3.5result <-  foreach(mc = 1:100, .combine = cbind,.packages = c("VGAM")) %dopar% one_iteration_example1(location = 2,mu00 = 2.5)
result1<-cbind(result1,Reduce("+",l1m3.5result[1,])/100,Reduce("+",l1m3.5result[3,])/100,Reduce("+",l1m3.5result[5,])/100)
result2<-cbind(result2,Reduce("+",l1m3.5result[2,])/100,Reduce("+",l1m3.5result[4,])/100,Reduce("+",l1m3.5result[6,])/100)
result3<-cbind(result3,Reduce("+",l1m3.5result[7,])/100,Reduce("+",l1m3.5result[8,])/100)
...
set.seed(951004)
l1m4.5result <-  foreach(mc = 1:100, .combine = cbind,.packages = c("VGAM")) %dopar% tryCatch(one_iteration_example1(location = 2,mu00 = 3.5),error = function(e) {list(adapt_alpha    = rep(NA_real_, 30),adapt_power    = rep(NA_real_, 30),dp_bh_fdp      = rep(NA_real_, 30),dp_bh_power    = rep(NA_real_, 30),dp_adapt_alpha = rep(NA_real_, 30),dp_adapt_power = rep(NA_real_, 30),t11 = NA_real_,t22 = NA_real_)})
result1<-cbind(result1,Reduce("+",l1m4.5result[1,])/100,Reduce("+",l1m4.5result[3,])/100,Reduce("+",l1m4.5result[5,])/100)
result2<-cbind(result2,Reduce("+",l1m4.5result[2,])/100,Reduce("+",l1m4.5result[4,])/100,Reduce("+",l1m4.5result[6,])/100)
result3<-cbind(result3,Reduce("+",l1m4.5result[7,])/100,Reduce("+",l1m4.5result[8,])/100)
...
set.seed(951004)
l1m5.5result <-  foreach(mc = 1:100, .combine = cbind,.packages = c("VGAM")) %dopar% tryCatch(one_iteration_example1(location = 2,mu00 = 4.5),error = function(e) {list(adapt_alpha    = rep(NA_real_, 30),adapt_power    = rep(NA_real_, 30),dp_bh_fdp      = rep(NA_real_, 30),dp_bh_power    = rep(NA_real_, 30),dp_adapt_alpha = rep(NA_real_, 30),dp_adapt_power = rep(NA_real_, 30),t11 = NA_real_,t22 = NA_real_)})
result1<-cbind(result1,Reduce("+",l1m5.5result[1,])/100,Reduce("+",l1m5.5result[3,])/100,Reduce("+",l1m5.5result[5,])/100)
result2<-cbind(result2,Reduce("+",l1m5.5result[2,])/100,Reduce("+",l1m5.5result[4,])/100,Reduce("+",l1m5.5result[6,])/100)
result3<-cbind(result3,Reduce("+",l1m5.5result[7,])/100,Reduce("+",l1m5.5result[8,])/100)

write.csv(to.csv(l1m3.5result),"./result/l1m3.5result2.csv")
write.csv(to.csv(l1m4.5result),"./result/l1m4.5result2.csv")
write.csv(to.csv(l1m5.5result),"./result/l1m5.5result2.csv")

write.csv(result1,"./result/let_seting_example2_fdr.csv")
write.csv(result2,"./result/let_seting_example2_power.csv")

result1<-NULL
result2<-NULL
###alpha case mu00=2.5,location=3
set.seed(951004)
l1m3.5result <-  foreach(mc = 1:100, .combine = cbind,.packages = c("VGAM")) %dopar% tryCatch(one_iteration_example1(location = 3,mu00 = 2.5),error = function(e) {list(adapt_alpha    = rep(NA_real_, 30),adapt_power    = rep(NA_real_, 30),dp_bh_fdp      = rep(NA_real_, 30),dp_bh_power    = rep(NA_real_, 30),dp_adapt_alpha = rep(NA_real_, 30),dp_adapt_power = rep(NA_real_, 30),t11 = NA_real_,t22 = NA_real_)})
result1<-cbind(result1,Reduce("+",l1m3.5result[1,])/100,Reduce("+",l1m3.5result[3,])/100,Reduce("+",l1m3.5result[5,])/100)
result2<-cbind(result2,Reduce("+",l1m3.5result[2,])/100,Reduce("+",l1m3.5result[4,])/100,Reduce("+",l1m3.5result[6,])/100)
result3<-cbind(result3,Reduce("+",l1m3.5result[7,])/100,Reduce("+",l1m3.5result[8,])/100)
...
set.seed(951004)
l1m4.5result <-  foreach(mc = 1:100, .combine = cbind,.packages = c("VGAM")) %dopar% tryCatch(one_iteration_example1(location = 3,mu00 = 3.5),error = function(e) {list(adapt_alpha    = rep(NA_real_, 30),adapt_power    = rep(NA_real_, 30),dp_bh_fdp      = rep(NA_real_, 30),dp_bh_power    = rep(NA_real_, 30),dp_adapt_alpha = rep(NA_real_, 30),dp_adapt_power = rep(NA_real_, 30),t11 = NA_real_,t22 = NA_real_)})
result1<-cbind(result1,Reduce("+",l1m4.5result[1,])/100,Reduce("+",l1m4.5result[3,])/100,Reduce("+",l1m4.5result[5,])/100)
result2<-cbind(result2,Reduce("+",l1m4.5result[2,])/100,Reduce("+",l1m4.5result[4,])/100,Reduce("+",l1m4.5result[6,])/100)
result3<-cbind(result3,Reduce("+",l1m4.5result[7,])/100,Reduce("+",l1m4.5result[8,])/100)
...
set.seed(951004)
l1m5.5result <-  foreach(mc = 1:100, .combine = cbind,.packages = c("VGAM")) %dopar% tryCatch(one_iteration_example1(location = 3,mu00 = 4.5),error = function(e) {list(adapt_alpha    = rep(NA_real_, 30),adapt_power    = rep(NA_real_, 30),dp_bh_fdp      = rep(NA_real_, 30),dp_bh_power    = rep(NA_real_, 30),dp_adapt_alpha = rep(NA_real_, 30),dp_adapt_power = rep(NA_real_, 30),t11 = NA_real_,t22 = NA_real_)})
result1<-cbind(result1,Reduce("+",l1m5.5result[1,])/100,Reduce("+",l1m5.5result[3,])/100,Reduce("+",l1m5.5result[5,])/100)
result2<-cbind(result2,Reduce("+",l1m5.5result[2,])/100,Reduce("+",l1m5.5result[4,])/100,Reduce("+",l1m5.5result[6,])/100)
result3<-cbind(result3,Reduce("+",l1m5.5result[7,])/100,Reduce("+",l1m5.5result[8,])/100)


write.csv(to.csv(l1m3.5result),"./result/l1m3.5result3.csv")
write.csv(to.csv(l1m4.5result),"./result/l1m4.5result3.csv")
write.csv(to.csv(l1m5.5result),"./result/l1m5.5result3.csv")


write.csv(result1,"./result/let_seting_example3_fdr.csv")
write.csv(result2,"./result/let_seting_example3_power.csv")
write.csv(result3,"./result/computation_time.csv")
