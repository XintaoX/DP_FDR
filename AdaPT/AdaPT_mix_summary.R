################################################################
## Other information of AdaPT object
################################################################

library(RColorBrewer)

corr.lfdr <- function(obj, data = NULL, ...){
  ...
  lfdr <- obj$lfdr
  algo <- obj$algos[[1]]
  if (length(obj$algos) > 1){
    warning("multiple algorithms detected! Use the first model by default.")
  }
  if (is.null(data)){
    data <- obj$data
  }
  ...
  oracle.est <- EM.mix(x, pvals, plow = 0, phigh = 1,
                       dist = dist, algo = algo,
                       params0 = obj$params[[m]], ...)
  oracle.lfdr <- lfdr.mix(pvals, dist, oracle.est$params)
  
  corr.lfdr <- apply(obj$lfdr, 2, function(lfdr){
    cor(lfdr, oracle.lfdr)
  })
  corr.lfdr <- data.frame(alpha = rev(alphas), corr = rev(corr.lfdr))
  
  return(corr.lfdr)
}

info.plot <- function(obj, alpha, title,
                      data = NULL,
                      xlab = "x",
                      disp.pmax = 0.2,
                      num.xbreaks = 3,
